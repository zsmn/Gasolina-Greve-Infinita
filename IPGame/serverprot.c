...
int main(void){
    enum conn_ret_t cone;
    do{
        cone = connectToServer(IP);
        if(cone==SERVER_DOWN){
            puts("erro de servidor, nao encontrado");
        }
        if(cone==SERVER_FULL){
            puts("erro de servidor, cheio");
        }
        if(cone==SERVER_CLOSED){
            puts("erro de servidor, fechado para conexoes");
        }
        if(cone==SERVER_TIMEOUT){
            puts("erro de servidor, demorou demais");
        }
    }while(cone!=SERVER_UP);
    printf("deu certo");
   ...
   al_attach_audio_stream_to_mixer(musica, al_get_default_mixer()); //começo a musica
	...
	short* posis=(short*)malloc(2*sizeof(short));
	posis[0]=posx;
	posis[1]=posy;
	int check;
	puts("esse troco eh bugado");
	do{ 
	    check=sendMsgToServer(posis,2*sizeof(short));
	}while(check!=2*sizeof(short));
	puts("ei mandei!");
    ...
    while (!sair){//entra no loop do jogo
        while (!al_is_event_queue_empty(fila_eventos)){//se  acontecer algo
            ...
            al_wait_for_event(fila_eventos, &evento);//esse evento fica na fila
                ...
                desenha=1;
                do{ 
	                check=sendMsgToServer(mov,sizeof(char));
	            }while(check!=2*sizeof(short));
                recvMsgFromServer(posis,WAIT_FOR_IT);
                posx=posis[0];
                posy=posis[1];
