...
void tiros(int xo, int yo, int pox, int poy, int id, int posibala[6][960][2]);
int main(void){
            ...
            al_wait_for_event(fila_eventos, &evento);//esse evento fica na fila
                ...
                if(perm==0){
                    if(evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP && perm==0){
                        ...
                        tiros(evento.mouse.x,evento.mouse.y,posx*16,posy*16,testando,bala);
                        perm=1;
                ...
                if(perm==1){
                    ...
                    if(bala[testando][i][0]!=2){
                        if(matrizOcupada[(bala[testando][i+1][1])/16][(bala[testando][i+1][0])/16]=='1' && i>1){
                            perm=0;
                            i=0;
